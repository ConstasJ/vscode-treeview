...
export class FileTreeProvider implements vscode.TreeDataProvider<FileTreeItem>, vscode.TreeDragAndDropController<FileTreeItem> {
  ...
  // 批量添加文件到文件夹
  public async addMultipleFilesToFolder(parent: FileTreeItem, files: vscode.Uri[]): Promise<void> {
    if (parent.type !== NodeType.FOLDER) {
      vscode.window.showErrorMessage('只能在文件夹下添加文件');
      return;
    }
  ...
  private searchFileNode(uri: vscode.Uri, parentNode?: FileTreeItem): FileTreeItem | null {
  ...
  public async handleDrop(
    ...
    // 处理拖放的 URI 资源（文件或文件夹）
    try {
      const uriList: vscode.Uri[] = (JSON.parse(await transferItem.asString()) as Record<string, any>[]).map(uri => {
      ...
      if (!target) {
        for (const uri of uriList) {
          if (await this.isDirectory(uri)) {
            ...
            // 如果是文件，提示需要拖到一个文件夹节点
            vscode.window.showInformationMessage('请将文件拖到一个文件夹节点上');
          }
        }
        ...
        for (const uri of uriList) {
          if (await this.isDirectory(uri)) {
            ...
            if (exists) {
              vscode.window.showInformationMessage(`文件 "${fileName}" 已存在于该文件夹中`);
            } else {
              const fileNode = new FileTreeItem(
                fileName,
                NodeType.FILE,
  ...
  public async handleDrag(
    ...
    const uriList = source
      .filter(item => item.type === NodeType.FOLDER || item.type === NodeType.FILE)
      .map(item => item.filePath ? vscode.Uri.file(item.filePath) : undefined)
      .filter(uri => uri) as vscode.Uri[];
      
    dataTransfer.set('application/vnd.code.tree.explorer',
